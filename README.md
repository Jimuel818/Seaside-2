...
if ($reservation_id) {
    try {
        ...
        if (empty($cart_items_checkout) && !isset($_SESSION['success_message'])) {
            ...
            $reservation_fee = $total_checkout_price * $reservation_fee_percentage;
            $grand_total_with_fee = $total_checkout_price + $reservation_fee;
            $_SESSION['final_checkout_total_with_fee'] = $grand_total_with_fee; // Store the total with fee in session
            $can_proceed = true;
        }
